...
def get_activation_layer(activation_str: str):
    activation_layers = {'relu': nn.ReLU, 'prelu': nn.PReLU, 'leaky-relu': nn.LeakyReLU, 'sigmoid': nn.Sigmoid, 'tanh': nn.Tanh}
...
def get_activation_fn(activation_str: str):
    activation_fns = {'relu': torch.relu, 'prelu': torch.prelu, 'leaky-relu': F.leaky_relu, 'sigmoid': torch.sigmoid, 'tanh': torch.tanh}
