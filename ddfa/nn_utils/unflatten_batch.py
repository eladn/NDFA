...
def unflatten_batch(flattened_data: torch.Tensor, examples_indices: torch.LongTensor) \
    ...
    # TODO: set device like input for new created tensors!
    batch_range = torch.arange(1, examples_indices.size()[0] + 1)
    ...
    mask = nr_items_per_example.unsqueeze(-1).expand(batch_size, max_nr_items_per_example) >= torch.arange(1, max_nr_items_per_example + 1).unsqueeze(0).expand(batch_size, max_nr_items_per_example)
